(() => {
  ...
  const textTranslations = new Map([
    ...
    ['Stock Policy A 股量化投研工具链', 'Stock Policy A-Share Quantitative Research Toolkit'],
    ['开源量化研究与 AI Agent 工程', 'Open-Source Quantitative Research and AI Agent Engineering'],
    ['围绕 A 股、ETF 与指数搭建 Python/Tushare 数据管线，统一前复权指标口径，支持日线、资金流、财务数据预加载及 HDF5 回测数据构建。', 'Built Python and Tushare data pipelines for A-shares, ETFs, and indices, with consistent forward-adjusted indicators, preloading for daily bars, money flow, and fundamentals, and HDF5 dataset generation for backtesting.'],
    ['实现多股回测引擎，按 T 日收盘决策、T+1 开盘成交，纳入 100 股整手、先卖后买、可用现金、佣金和印花税，并自动输出交易明细、报告与收益曲线。', 'Implemented a multi-asset backtesting engine using close-of-day decisions and next-day open execution, with 100-share board lots, sell-before-buy ordering, available-cash checks, commissions, and stamp duty, automatically exporting transactions, reports, and return curves.'],
    ['构建文件驱动的 AI 预测—到期复盘—历史评估闭环，支持股票、ETF、指数并发任务，记录知识库版本，统计 1/3/5 日方向与区间结果，并可分段同步 Notion。', 'Built a file-driven AI prediction, post-horizon review, and historical evaluation loop for concurrent stock, ETF, and index tasks, with knowledge-base version tracking, 1/3/5-day direction and range evaluation, and chunked Notion synchronization.'],
    ['集成同花顺、韭研公社与博查新闻采集，并提供实验性的 Windows 持仓 OCR/视觉识别与条件交易计划；公开分支包含 40 个 pytest 测试用例和脱敏配置。', 'Integrated news collection from 10jqka, Jiuyangongshe, and Bocha, plus experimental Windows portfolio OCR/vision parsing and conditional trade planning; the public branch includes 40 pytest test cases and a sanitized configuration template.'],
    ...
    ['智慧家庭能量管理 TD-MPC 世界模型', 'TD-MPC World Model for Smart Home Energy Management'],
    ['南京邮电大学自动化学院 · 硕士阶段学术研究', 'Nanjing University of Posts and Telecommunications · Master\'s Research'],
    ['围绕光伏、电池、HVAC、刚性负荷和主电网构建智慧家庭能量管理环境，在多源不确定性和热舒适约束下建模运行成本最小化问题。', 'Modeled a smart-home energy management environment spanning photovoltaics, battery storage, HVAC, inflexible loads, and the utility grid, minimizing operating cost under multi-source uncertainty and thermal-comfort constraints.'],
    ['基于 TD-MPC 构建由编码器、动力学、奖励与 Q 值网络组成的隐性世界模型，将 MPPI 滚动规划嵌入训练，并设计动作安全层满足电池与温度物理约束。', 'Built a TD-MPC latent world model comprising encoder, dynamics, reward, and Q-value networks; embedded receding-horizon MPPI planning into training and added an action-safety layer for battery and temperature constraints.'],
    ['使用 Pecan Street 真实家庭数据，以 5 组随机种子对比规则、MPPI、DDPG 与 MBRL；总运行成本降低', 'Using real household data from Pecan Street and five random seeds, benchmarked against rule-based control, MPPI, DDPG, and MBRL; reduced total operating cost by'],
    ['，平均舒适温度偏离降低', ' and mean thermal-comfort deviation by'],
    ['测试阶段直接由策略网络决策，单步平均耗时', 'During testing, the policy network acted directly with an average per-step latency of'],
    ['；成果形成第一作者论文并获《信息与控制》录用（2026.04）。', '; the work led to a first-author paper accepted by Information and Control in April 2026.'],
    ['论文官网 ↗', 'Paper Page ↗'],
    ['科研成果页 →', 'Publication Details →'],
    ...
    ['不限', 'Flexible'],
    ['籍贯', 'Hometown'],
    ['江苏 · 苏州 · 张家港', 'Zhangjiagang · Suzhou · Jiangsu'],
    ['技术博客 ↗', 'Technical Blog ↗'],
